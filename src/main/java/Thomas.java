        ...
        while (userInput.hasNextLine()) {
            try {
                ...
                if (keyword.equals("bye")) {
                    ...
                    // size() is the count of tasks that actually exist, so an
                    // out-of-range number is reported rather than reaching get().
                    if (markNumber < 1 || markNumber > tasks.size()) {
                        throw new ThomasException("There is no task " + markNumber + "! You only have "
                                + tasks.size() + " task(s).");
                    }
                    Task taskToMark = tasks.get(markNumber - 1);
                    ...
                    Task taskToUnmark = tasks.get(unmarkNumber - 1);
                    taskToUnmark.unmarkAsDone();
                    printBlock("OK, I've marked this task as not done yet:", "   " + taskToUnmark);
                } else if (keyword.equals("delete")) {
                    if (parts.length < 2 || parts[1].isBlank()) {
                        throw new ThomasException("HEYY!! You need a valid number to delete");
                    }
                    int deleteNumber;
                    try {
                        deleteNumber = Integer.parseInt(parts[1].trim());
                    } catch (NumberFormatException e) {
                        throw new ThomasException("WHAT? Why are you passing a non integer?! Give me an INTEGER!!");
                    }
                    if (deleteNumber < 1 || deleteNumber > tasks.size()) {
                        throw new ThomasException("There is no task " + deleteNumber + "! You only have "
                                + tasks.size() + " task(s).");
                    }
                    ...
                    if (keyword.equals("todo")) {
                        ...
                        newTask = new TodoTask(parts[1].trim());
